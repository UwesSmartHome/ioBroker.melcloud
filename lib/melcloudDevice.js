...
class MelcloudDevice {
	...
	getDeviceInfo(callback, deviceOption, value) {
		...
		if (gthis.currentRequestExecution < 1) {
			...
			const url = "https://aquarea-smart.panasonic.com/" + gthis.id + "&buildingID=" + gthis.buildingId;
			const method = "get";
		...
		switch (deviceOption) {
			...
			case commonDefines.DeviceOptions.VaneVerticalDirection:
				if (value < 0 || value > 5 && value != 7) {
					gthat.log.info("VaneVerticalDirection: unsupported value '" + value + "' - falling back to '0'!");
					value = 0;
				}
				return value;
